import type { Connection, GenericData, Snapshot } from '../../src/modules/connection/connection'
...
export const compareDataMap = (snapshotList: Snapshot[], point: GenericData, moment: Date) => {
	...
	for (const snapshot of snapshotList) {
		...
		if (snapshot.data['_measurement'] !== indexer) {
			continue
		}
...
export const compareSnapshotInRange = async (
	...
	for (const [i, bufferValue] of snapshotList.entries()) {
	...
	return {
		mockConnection: {
			getLastDataFrom: async (date, indexerList) => {
				...
				for (const snapshot of snapshotList) {
					if (snapshot.timestamp > timestampDate) {
						break
					}

					const id = snapshot.data._measurement as string
					if (!indexerList.includes(id)) {
						continue
					}
			...
			getDataFromRange: async (date1, date2, indexerList) => {
				...
				for (const snapshot of snapshotList) {
					if (snapshot.timestamp <= timestampDate1) {
						continue
					}
					if (snapshot.timestamp > timestampDate2) {
						break
					}
